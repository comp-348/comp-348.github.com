...
*** ~setq~
The ~setq~ tool means "set quoted". It's similar to ~set~, except that we don't have to quote the symbol
being passed to it: 
...
Will do the same thing. However, we can /also/ use ~setf~ to set the values /inside/ a list:
