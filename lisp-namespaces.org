...
One problem that I've seen a lot is confusion between using ~list~ and ~(list)~. The two are very
different and knowing the difference between them is important.
...
This is the exact same thing we have going on Lisp. In Lisp we have 2 namespace: a function and a
