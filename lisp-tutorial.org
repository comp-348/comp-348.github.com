...
take the form ~(function-name arg-1 arg-2 ...)~. That's the whole syntax for the language. For
example, to define a linked list, we use the ~list~ function, like so: 
...
We can use these 3 functions, ~car~, ~cdr~ and ~cons~ to take apart lists and put them back together
...
Here, we have a single tree who's root node is ~7~. It has a left sub-tree and a right sub-tree, both of
